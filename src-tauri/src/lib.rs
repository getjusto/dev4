...
fn get_child_processes(parent_pid: u32) -> Vec<u32> {
    println!("=======================================");
    println!(
        "[get_child_processes] Starting search for children of PID: {}",
        parent_pid
    );
    ...
    println!(
        "[get_child_processes] Found {} total processes in system",
        all_processes.len()
    );
    ...
    fn collect_children(
        ...
        println!(
            "[collect_children] Finding immediate children of PID: {}",
            pid
        );
        ...
        println!(
            "[collect_children] PID {} - Full process list contains {} processes",
            pid,
            all_processes.len()
        );
        ...
        println!(
            "[collect_children] Found {} processes with PPID={}",
        ...
        for (i, &(proc_pid, proc_ppid, ref cmd)) in all_processes.iter().take(700).enumerate() {
            ...
            println!(
                "[collect_children] {}Process {}: PID={}, PPID={}, IsChild={}, CMD={}",
                prefix, i, proc_pid, proc_ppid, is_child, cmd
            );
        }

        println!(
            "[collect_children] Found {} immediate children for PID: {}",
        ...
        for &child_pid in &immediate_children {
            println!(
                "[collect_children] Adding child PID: {} to results",
                child_pid
            );
            all_children.push(child_pid);
            println!(
                "[collect_children] Recursively collecting children of PID: {}",
                child_pid
            );
    ...
    fn get_all_processes() -> Vec<(u32, u32, String)> {
        ...
        if let Ok(output_str) = String::from_utf8(output.stdout) {
            ...
            for (i, &(pid, ppid, ref cmd)) in processes.iter().take(10).enumerate() {
                println!(
                    "[get_all_processes] Process {}: PID={}, PPID={}, CMD={}",
                    i, pid, ppid, cmd
                );
            }

            println!(
                "[get_all_processes] Total processes found: {}",
                processes.len()
            );
    ...
    println!(
        "[get_child_processes] Starting recursive collection for PID: {}",
        parent_pid
    );
    collect_children(parent_pid, &mut all_children, &mut visited, &all_processes);
    println!(
        "[get_child_processes] Total descendant processes found: {}",
        all_children.len()
    );
    if !all_children.is_empty() {
        println!(
            "[get_child_processes] All descendant PIDs: {:?}",
            all_children
        );
    }
    println!("=======================================");
...
fn kill_process(pid: u32) -> bool {
    ...
    let term_result = Command::new("kill").arg(pid.to_string()).output();

    match &term_result {
        Ok(_) => println!("[kill_process] SIGTERM sent successfully to PID: {}", pid),
        Err(e) => println!(
            "[kill_process] Failed to send SIGTERM to PID {}: {}",
            pid, e
        ),
    }
    ...
    let is_running = Command::new("ps")
        ...
        .map(|output| {
            ...
            println!(
                "[kill_process] PID {} running status: {}",
                pid, still_running
            );
            still_running
        })
        .unwrap_or_else(|e| {
            println!(
                "[kill_process] Error checking if PID {} is running: {}",
                pid, e
            );
            false
        });
    ...
    if is_running {
        ...
        match &kill_result {
            ...
            Err(e) => println!(
                "[kill_process] Failed to send SIGKILL to PID {}: {}",
                pid, e
            ),
        }
        ...
        println!(
            "[kill_process] Process PID {} terminated with SIGTERM, result: {}",
            pid, success
        );
        success
    }
        ...
        .run(|_app_handle, event| match event {
            RunEvent::Exit => {
                ...
                if !remaining_pids.is_empty() {
                    println!(
                        "Found {} remaining child processes, attempting to kill them",
                        remaining_pids.len()
                    );
