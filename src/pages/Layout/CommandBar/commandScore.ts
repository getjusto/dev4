// Based on Superhuman's command bar
...
export function commandScore(string: string, query: string) {
   ...
   */
  return commandScoreInner(string, query, formatInput(string), formatInput(query), 0, 0, {})
}

export function commandScoreFilterAndSort(items: any[], fields: string[], query: string) {
  return items
    .map(item => {
      const scores: number[] = []
      ...
      for (const field of fields) {
        ...
        if (Array.isArray(value)) {
          for (const subValue of value) {
            const score = commandScore(subValue, query)
            scores.push(score)
          }
        } else if (value) {
